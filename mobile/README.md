...
**사용자 전용 앱**이다. 관리자 화면은 이 앱에 없다 —
연구 운영자는 별도 사이트(`admin-web/`)를 브라우저로 연다.

밤에 자기 전에 여는 앱이라 **남색 밤하늘 톤 한 가지**로만 그린다(모드 전환 없음).
색·모서리·간격은 `src/theme.ts` 한 곳에서 관리한다.
...
```
app/
  index.tsx              시작 화면 — 닉네임 + 비밀번호 (회원가입 / 로그인)
  user/home.tsx          홈 — 시작 버튼, 기기 연결 상태, 목표 온도, 히터 상태,
                         지난밤 결과, 수면 입면 분석, 어젯밤 평가 팝업
  user/records.tsx       기록 — 달력에서 날짜를 고르면 입면 성공 여부·입면시간·수면 평가
  user/analysis.tsx      수면 입면 분석 — 지표(입면시간/잠든 시간/적정 온도) × 기간(일별/주별/월별)
                         꺾은선 한 장. 시안 구조를 그대로 따른다
```

### 시작 버튼과 기기 상태

- 시작 버튼을 누르면 `POST /api/devices/{id}/commands` 로 명령이 올라가고,
  PC 의 브리지가 가져가 기기 시리얼에 `start` 를 넣는다.
- 버튼 아래 상태 칩이 색으로 알린다 —
  초록 `기기 연결됨` / 주황 `기기 응답 없음(전원·배터리)` / 빨강 `기기 연결 안 됨`.
  칩을 누르면 기기 연결 시트가 열린다.
- 센서 워밍업 20초가 끝나 값이 정상으로 들어오면(서버의 `WARMUP_DONE`)
  `expo-speech` 로 **"연결이 확인되었습니다. 수면케어를 시작합니다."** 를 읽어준다.
- 어젯밤 평가 팝업은 **입면이 확정됐을 때만** 화면 가운데 뜨고, 평가를 남겨야 닫힌다.
- 피부 온도 같은 원자료는 사용자 화면에 노출하지 않는다(관리자 화면에서만 본다).
- 분석 화면은 시안에 있는 것만 둔다 — 심박수·온도별 차트처럼 시안에 없는 그래프는 넣지 않는다.
  꺾은선은 템플릿의 달빛 노랑(`theme.moon`)으로 그리고 오른쪽에 단위선 라벨을 붙인다.

서버 주소는 `app.json` 의 `extra.defaultServerUrl` 로 빌드에 넣는다.
현장에서 바꿔야 하면 시작 화면의 로고를 길게 눌러 주소 칸을 연다.

- 가입/로그인 시 받은 접근 토큰과 사용자 ID / 기기 ID / 서버 주소가 `AsyncStorage` 에 저장되어
