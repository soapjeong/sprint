...
입면 성공 여부 / 실패 원인 / 시작 누른 시간 / 입면 성공 시간 / 입면시간 /
평점·특이사항 / 목표 온도·안정심박수. 맨 위에는 사람별·전체 "기기 사용 대비 입면 성공 비율"이 있다.

입면에 실패한 경우 펌웨어가 원인을 함께 보낸다(`NO_ONSET` 의 두 번째 값):
`hr_high`(심박이 기준보다 높게 유지) / `motion`(움직임 감지) / `sensor`(센서 데이터 오류) / `unknown`.
