...
// ------- 히터 출력 램프(파워 비례 점화) -------
// PID가 요구하는 duty로 한 번에 점프하지 않고, 초당 이 값만큼만 증감시켜
// 히터가 "파워에 비례해서" 서서히 켜지고 꺼지도록 한다(급격한 풀파워 점프 방지).
static const float DUTY_RAMP_PER_SEC = 60.0f;  // 0~255 기준, 초당 최대 변화량
...
// 센서 NaN/범위 이상, 스파이크처럼 "노이즈일 수 있는" 이상치는 이 횟수만큼
// 연속으로 관측되어야 FAULT로 확정한다(단발성 노이즈로 인한 오작동 방지).
// 단, 화상 방지용 절대 온도 상한(SKIN/HEATER_HARD_LIMIT_C)은 안전을 위해
// 디바운스 없이 즉시 차단한다.
static const int FAULT_CONFIRM_COUNT = 5;
...
// ------- 심박수(HR) 계산 -------
// 개별 비트마다 순간 BPM을 내던 방식은 노이즈에 취약하므로,
// 5초 창(window) 동안 검출된 비트 수를 세어 BPM을 산출해 안정성을 높인다.
static const unsigned long HR_WINDOW_MS = 5000UL;
...
// 히터 출력 램프(파워 비례 점화) 내부 상태 — PWM duty의 연속값 누적기
static float currentDutyF = 0.0f;
...
// MAX30102 비트 검출 / 5초 창 BPM 산출용
static long  lastBeatMs = 0;
static float latestBpm = 0;
static unsigned long hrWindowStartMs = 0;
static int   hrWindowBeatCount = 0;
...
//  절대 상한(화상 방지)은 즉시 차단하되, 센서 NaN/범위이상·급상승(스파이크)처럼
//  노이즈일 수 있는 이상치는 FAULT_CONFIRM_COUNT회 연속 관측되어야 FAULT로 확정한다.
// ===========================================================================
static SafetyState evaluateSafety(float skinC, float heaterC,
                                   float lastSkinC, float lastHeaterC,
                                   SafetyState current) {
  static SafetyState pendingState = STATE_NORMAL;
  static int         pendingCount = 0;

  // 이미 래치된 FAULT는 재가동(REARM) 조건 전까지 유지
  if (current != STATE_NORMAL) return current;

  // 절대 상한(화상 방지) — 디바운스 없이 즉시 차단
  if (!isnan(skinC)   && skinC   >= SKIN_HARD_LIMIT_C)   return STATE_FAULT_OVERTEMP_SKIN;
  if (!isnan(heaterC) && heaterC >= HEATER_HARD_LIMIT_C) return STATE_FAULT_OVERTEMP_HEATER;

  // 센서 이상 / 급상승(스파이크) 후보 판정 — 노이즈일 수 있으므로 연속 관측 필요
  SafetyState candidate = STATE_NORMAL;
  if (isnan(skinC) || skinC < T_SENSE_MIN_C || skinC > T_SENSE_MAX_C) {
    candidate = STATE_FAULT_SENSOR;
  } else if (isnan(heaterC) || heaterC < T_SENSE_MIN_C || heaterC > T_SENSE_MAX_C) {
    candidate = STATE_FAULT_SENSOR;
  } else if (!isnan(lastSkinC) && (skinC - lastSkinC) >= SPIKE_JUMP_C) {
    candidate = STATE_FAULT_SPIKE_SKIN;
  } else if (!isnan(lastHeaterC) && (heaterC - lastHeaterC) >= SPIKE_JUMP_C) {
    candidate = STATE_FAULT_SPIKE_HEATER;
  }

  if (candidate == STATE_NORMAL) {
    pendingState = STATE_NORMAL;
    pendingCount = 0;
    return STATE_NORMAL;
  }

  // 같은 종류의 이상치가 계속될 때만 카운트, 종류가 바뀌면 새로 카운트
  if (candidate == pendingState) pendingCount++;
  else { pendingState = candidate; pendingCount = 1; }

  if (pendingCount >= FAULT_CONFIRM_COUNT) return pendingState; // FAULT 확정 및 래치

  return STATE_NORMAL; // 아직 확정 전 — 정상 유지
}
...
static void resetPID() {
  pidIntegral = 0; pidPrimed = false; dbgP = dbgI = dbgD = 0;
  currentDutyF = 0.0f; // 히터 램프 상태도 함께 초기화(재가동 시 다시 서서히 점화)
}
...
static void pollMotionAndHR() {
  ...
  // ---- MAX30102: 비트 검출 → 5초 창(window) 동안 박동 수를 세어 BPM 산출 ----
  //      (비트 1개마다 순간 BPM을 내면 노이즈에 취약하므로, 5초간 누적 개수로
  //       평균해 안정적인 BPM을 얻는다)
  if (g_maxOk) {
    ...
    if (checkForBeat(irValue)) {
      ...
      lastBeatMs = nowMs;
      // 235ms 미만 간격(=255bpm 초과)은 노이즈로 간주하고 버림
      if (delta > 235) hrWindowBeatCount++;
    }
  }

  unsigned long nowMs = millis();
  if (nowMs - hrWindowStartMs >= HR_WINDOW_MS) {
    float windowSec = (nowMs - hrWindowStartMs) / 1000.0f;
    if (hrWindowBeatCount > 0 && windowSec > 0.0f) {
      float bpm = hrWindowBeatCount * (60.0f / windowSec);
      latestBpm = bpm;
      epochHrSum += (long)(bpm + 0.5f);
      epochHrCount++;
    }
    hrWindowBeatCount = 0;
    hrWindowStartMs = nowMs;
  }
}
...
static void startSession() {
  ...
  hrWindowStartMs = sessionStartMs;
  hrWindowBeatCount = 0;
...
void setup() {
  ...
  Serial.println("min");
  Serial.print("# FaultConfirmCount="); Serial.print(FAULT_CONFIRM_COUNT);
  Serial.print("  DutyRampPerSec="); Serial.println(DUTY_RAMP_PER_SEC,0);
...
void loop() {
  ...
  if (now - lastControlMs >= CONTROL_PERIOD_MS) {
    ...
    if (heaterAllowed) {
      ...
      // 히터파워(duty)가 PID 목표치로 즉시 튀지 않고, 초당 정해진 양만큼만
      // 증감하도록 램프를 걸어 "파워에 비례해서" 서서히 켜지고 꺼지게 한다.
      float maxStep = DUTY_RAMP_PER_SEC * (CONTROL_PERIOD_MS / 1000.0f);
      if (out > currentDutyF + maxStep)      currentDutyF += maxStep;
      else if (out < currentDutyF - maxStep) currentDutyF -= maxStep;
      else                                   currentDutyF = out;

      duty = (int)(currentDutyF + 0.5f);
      lastSkinTempC = skinC; pidPrimed = true;
    } else {
      resetPID();
    }
    if (safetyState != STATE_NORMAL) { duty = 0; currentDutyF = 0.0f; }  // ★ 하드 컷오프 강제 ★
    pwmWrite(duty);
